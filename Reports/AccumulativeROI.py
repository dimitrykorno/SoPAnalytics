...
def new_report(os_list=["iOS"],
    ...
    for os_str in os_list:
        ...
        def flush_user_data():
            ...
            for d0 in user_dau.keys():
                if user_dau[d0] is not None:
                    dau[publisher][source][d0]["Users"] += 1
                    dau[publisher][source][d0]["Revenue"] += user_dau[d]
                    # if len(user_transactions) > 0:
        ...
        while Report.get_next_event():
            ...
            day_in_game = Report.get_time_since_install(measure="d")
        ...
        for publisher in sources.keys():
            ...
            writer = pd.ExcelWriter(
                "Results/Отчёт по трафику/" + OS.get_os_string(os) + " " + publisher + " Cummulative ROI.xlsx")
            ...
            for source in sources[publisher].keys():
                ...
                if get_cpi(source) != 0:
                    ...
                    Report.draw_plot(range(0, days_since_install + 1),
                                     ...
                                     show=False,
                                     folder="Results/Отчёт по трафику")
                    Report.draw_plot(range(0, days_since_install + 1),
            ...
            plt.title(title)
            plt.savefig("Results/Отчёт по трафику/" + title + ".png", bbox_inches='tight')
