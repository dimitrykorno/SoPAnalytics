...
class Report(Singleton):
    ...
    def __init__(self, sql_events,
        ...
        if get_installs:
            Report.installs = get_installs_list(os=OS.ios,
    ...
    def get_next_user(self):
        ...
        if self.is_new_user(next_id1=user_id1, next_id2=user_id2):
            ...
            # новым текущим пользователем
                ...
                if Report.installs:
                    new_user.install_date, new_user.publisher, new_user.source, new_user.installed_app_version, new_user.country = Report._get_install_data(
                        user_id1, user_id2)
                else:
    ...
    def _get_install_data(user_id1, user_id2):
        ...
        for install in Report.installs:
            if user_id2 != "" and user_id2 in install[OS.get_id(Report.os)] or \
                ...
                return install["install_datetime"].date(), publisher_name, install["tracker_name"], install[
                    "app_version_name"], install["country_iso_code"]
